...
- schema را با فرمت JSON-LD بنویس و با داده‌ی واقعی صفحه پر کن — schema با داده‌ی جعلی
  نقض دستورالعمل گوگل است و می‌تواند جریمه بیاورد. **کدام نوع، کدام فیلد، و چطور اعتبارسنجی
  شود: `docs/schema-guide.md`.** فیلدی که داده‌اش را نداری ننویس؛ اسکیمای ناقصِ درست بهتر از
  اسکیمای کاملِ ساختگی است.
