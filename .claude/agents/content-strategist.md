...
هر brief در فایل خودش ذخیره می‌شود — یک فایل به‌ازای هر مقاله:

```
projects/<slug>/content/NN-<keyword-slug>.md
```

قالب کامل (با بخش‌های SERP، رقبا، اسکیما و گیت کیفیت که ایجنت‌های دیگر پر می‌کنند)
در `docs/workflow.md` فاز ۵ است. تو مالک بخش «ساختار» هستی، اما فایل را تو می‌سازی و
جای بقیه را با عنوان خالی نگه می‌داری تا هر ایجنت سهم خودش را بنویسد.

حداقلِ چیزی که خودت باید بنویسی:
